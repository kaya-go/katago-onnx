...
class NormMask(torch.nn.Module):
    def __init__(
        ...
        self.use_gamma = (
            ...
            or (
                (self.norm_kind == "fixup" or self.norm_kind == "fixscale" or self.norm_kind == "fixscaleonenorm")
                and fixup_use_gamma
            )
            or force_use_gamma
        )
...
class Model(torch.nn.Module):
    ...
    def postprocess_single_heads_output(self, outputs):
        ...
        pred_scorestdev = (
            SoftPlusWithGradientFloorFunction.apply(out_miscvalue[:, 1], 0.05, False) * self.scorestdev_multiplier
        )
        pred_lead = out_miscvalue[:, 2] * self.lead_multiplier
        pred_variance_time = (
            SoftPlusWithGradientFloorFunction.apply(out_miscvalue[:, 3], 0.05, False) * self.variance_time_multiplier
        )
        if self.config["version"] < 14:
