...
class GameState:
    ...
    def get_model_outputs(
        self, model: "Model", sgfmeta: Optional[SGFMetadata] = None, extra_output_names: List[str] = []
    ):
        import torch

        from ..train.model_pytorch import ExtraOutputs, Model
        ...
        with torch.no_grad():
            ...
            if sgfmeta is not None:
                metarow = sgfmeta.get_metadata_row(
                    nextPlayer=self.board.pla, boardArea=self.board.x_size * self.board.y_size
                )
    ...
    def copy(self):
        new_gs = GameState(self.board_size, self.rules)
        new_gs.board = self.board.copy()
        new_gs.moves = self.moves.copy()
        new_gs.boards = [b.copy() for b in self.boards]
        new_gs.redo_stack = self.redo_stack.copy()
        return new_gs
